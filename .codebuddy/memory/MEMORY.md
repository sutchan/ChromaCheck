...
- 已从「纯文档 + 静态原型」进入**应用开发阶段**，v1.0.0 已实现（2026-09-04），当前版本 **v1.7.5**（2026-09-08；注意会话间隙用户会自行 bump/提交，改版本前必须重读 VERSION 并 `git status --short --untracked-files=all` 确认新建文件是否已被中间会话提交）。
...
- **字体（v1.7.4 起自托管）**：`app/layout.tsx` 用 `next/font/google` 注入 Archivo + IBM Plex Mono（`variable:--font-sans/--font-mono`），移除手动 Google Fonts `<link>`；站点域名统一由 `lib/site.ts` 的 `SITE_URL` 提供。构建时 next/font 需访问 fonts.gstatic.com，离线/弱网可能 `ECONNRESET`（Next 自动重试通常可过）。
- 版本单一来源：`VERSION`(=1.7.5) 与 `package.json` version 一致；原型文件头原为 v0.1.0，与正式应用版本解耦。
- **导航结构（v1.7.5）**：页眉 `components/layout/Navbar.tsx` 仅保留「首页 / 开始检测 / 历史」；「科普」「隐私」入口已移至页脚 `components/layout/Footer.tsx`（页脚含 开始检测/科普文章/隐私政策/GitHub，且展示版本号需随 bump 同步——曾长期停留在 v1.7.0 脱节，v1.7.5 修正）。
