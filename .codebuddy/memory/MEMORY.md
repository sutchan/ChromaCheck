...
- 已从「纯文档 + 静态原型」进入**应用开发阶段**，v1.0.0 已实现（2026-09-04），今日已迭代至 **v1.0.3**。
...
- 版本单一来源：`VERSION`(=1.0.3) 与 `package.json` version 一致；原型文件头原为 v0.1.0，与正式应用版本解耦。
...
- 文档治理教训：分册（ARCHITECTURE/DATA-SPEC/PRD/PRIVACY）的「规划蓝图」结构树/存储规范/上报功能须明确标注 v1.0 实际与 v1.1 规划边界，否则会与 SPEC/代码矛盾（v1.0.3 已统一：去 `advanced` 模式、path-tracking/hue-arrangement 标注未实现、`LocalStorageData` 标注 v1.1、`PRIVACY` 明确 v1.0 无上报）。分册头注释保持各自文档版，不随项目版本刷写。
