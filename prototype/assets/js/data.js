...
CC.articles = [
  { slug: 'what-is-color-blindness', title: '色盲和色弱，其实不是一回事', cat: '基础知识', read: '6 分钟', hue: '#6a7fc4', excerpt: '色盲是某类视锥细胞缺失，色弱是功能减弱。两者的成因、表现与生活影响差别很大。',
    body: [
      ['视锥细胞：色觉的起点', '视网膜上有三类视锥细胞，分别对长波（红）、中波（绿）、短波（蓝）最敏感。大脑比较三类细胞的信号强弱，才"算"出颜色。'],
      ['色盲与色弱，差别在程度', '色盲是某一类视锥细胞缺失，对应颜色会直接混淆（如红绿不分）；色弱是细胞功能减弱，只是分辨阈值升高——能分，但需要更大的色差。'],
      ['对生活的影响不同', '多数色弱者在日常场景下毫无障碍，仅在辨色精度要求高的任务（信号灯、线缆、医学影像）中吃力；色盲的影响则更明显。']
    ] },
  { slug: 'ishihara-principle',     title: '石原氏检测图是怎么"骗过"眼睛的', cat: '检测原理', read: '8 分钟', hue: '#c9903c', excerpt: '同样大小、同样明暗的色点，只靠色相差异构成数字——这是等亮度设计的巧思。',
    body: [
      ['关键在"等亮度"', '石原氏图里所有色点明暗一致、只差色相。这样既不能靠亮度猜，也不能靠轮廓猜，只有真正能分辨该色相差异的人才读得出数字。'],
      ['数字是怎么"藏"进去的', '前景点构成数字、背景点是干扰，两者色相互相接近但明度区间不同。正常色觉能据此分离数字与背景，异常色觉则把两者混为一谈。'],
      ['为什么红绿色觉异常会"读错"', '红绿两色的敏感曲线在光谱上高度重叠，异常者对它们之间的明度差不敏感，于是数字"消失"或读成别的数字。']
    ] },
  { slug: 'inheritance',            title: '色觉异常会遗传吗？一张图看懂规律', cat: '遗传规律', read: '5 分钟', hue: '#49b295', excerpt: '红绿色觉异常属于 X 连锁隐性遗传，男性患病率约为女性的 16 倍。',
    body: [
      ['基因在 X 染色体上', '红绿色觉异常属于 X 连锁隐性遗传。男性只有一条 X 染色体，只要携带致病基因就会表现；女性有两条 X，需两条都带才表现，故多为携带者。'],
      ['男女差异从哪来', '男性患病率约 8%，女性约 0.5%，相差约 16 倍。这也是为什么"色盲多为男性"的普遍印象成立。'],
      ['会怎么传给孩子', '色觉异常男性与正常女性结合，儿子全部正常、女儿全部为携带者；携带者女性与正常男性结合，儿子有约一半概率表现异常。']
    ] },
  { slug: 'career-limits',          title: '哪些专业和职业对色觉有明确要求', cat: '职业相关', read: '7 分钟', hue: '#bb5566', excerpt: '梳理高考体检、驾驶证申领与部分行业的色觉要求，附官方文件依据。',
    body: [
      ['高考体检有明确要求', '《普通高等学校招生体检指导意见》对美术类、医学类、交通运输类等专业的色觉提出要求，色觉异常可能限制部分专业报考。'],
      ['驾驶证申领', '红绿色盲不得申领机动车驾驶证；红绿色弱对部分准驾车型（如大型客车、牵引车）有限制，具体以最新规定为准。'],
      ['行业岗位的隐性门槛', '电工（线缆识别）、化工（试剂变色判断）、航空与航海（信号灯）、部分公安岗位通常要求正常色觉，入职前需确认。']
    ] },
  { slug: 'daily-tips',             title: '色觉异常者的 12 个生活小技巧',   cat: '生活贴士', read: '5 分钟', hue: '#3fa8b8', excerpt: '从挑衣服到读图表，用标签、明度和位置替代色相，可以解决大部分困扰。',
    body: [
      ['用标签代替颜色', '衣物、文件、线缆按文字或图标分类，而不是靠颜色区分；给储物盒贴名称标签，能避开"这两个颜色太像"的困扰。'],
      ['善用工具辅助', '手机取色类 App、色觉辅助滤镜（如电子设备的色彩校正模式）可以在关键场景帮你确认颜色。'],
      ['信任明度与位置', '看红绿灯看位置（上红下绿、横向左红右绿）而非颜色；看图表优先看形状、线型与数值标注，不只看色相。']
    ] },
  { slug: 'when-to-see-doctor',     title: '什么情况需要去医院做专业检查',   cat: '就医指南', read: '4 分钟', hue: '#87ad4c', excerpt: '在线筛查只是第一步，出现这些信号建议尽快到眼科做进一步检查。',
    body: [
      ['在线筛查只是第一步', '屏幕色域、环境光、显示器老化都会影响结果，在线检测只能作为初筛，不能替代专业诊断。'],
      ['出现这些信号该就医', '体检发现色觉异常、因职业需要出具证明、或自感辨色困难已影响生活与工作，都建议到眼科做进一步检查。'],
      ['医院怎么做', '标准光源箱 + 假同色图（石原氏）+ 色相排列（D15 / Farnsworth）+ 色觉镜（anomaloscope）综合判读，能区分色盲与色弱、定位异常轴。']
    ] }
];

/* ---------- 6.5 趣味性数据（章文案 / 称号 / 三轴科普）----------
   边界：答题中永不显示对错；称号为旅人隐喻、去污名化，仅本机展示，可随「趣味体验」开关关闭。 */

/* 检测章节（按题型分组，适配任意题集）：0 暖身 / 1 主体 / 2 深水 */
CC.chapters = [
